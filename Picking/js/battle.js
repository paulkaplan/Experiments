...
var Ball, BallView, Bird, BirdView, Block, BlockView, Body, Engine, View, animate, ball, clamp, engine, exponentialEaseOut, height, init, keyFromPos, onWindowResize, paused, render, stats, updatePhysics, width, world,
...
Engine = (function() {
  ...
  Engine.prototype.updatePhysics = function() {
    ...
    _.each(this.underControl, function(body) {
      ...
      body.decayAccel();
      return body.force.set(body.accel * Math.sin(body.rotation), body.accel * Math.cos(body.rotation), 0);
    });
...
BlockView = (function(_super) {
  ...
  function BlockView(type) {
    ...
    if (this.type === 'long') {
      this.geometry = new THREE.CubeGeometry(10, 30, 10);
    }
    if (this.type === 'longH') {
      this.geometry = new THREE.CubeGeometry(30, 10, 10);
    }
    this.material = new THREE.MeshNormalMaterial();
    this.mesh = new THREE.Mesh(this.geometry, this.material);
...
Block = (function(_super) {
  ...
  function Block(type) {
    ...
    if (this.type === 'long') {
      this.shape = new CANNON.Box(new CANNON.Vec3(5, 15, 5));
    }
    if (this.type === 'longH') {
      this.shape = new CANNON.Box(new CANNON.Vec3(15, 5, 5));
    }
