...
CANNON.Demo = function(){
  ...
  this.solidMaterial = new THREE.MeshLambertMaterial({map: THREE.ImageUtils.loadTexture( "textures/concrete.jpg" ) })
  this.planeMaterial = new THREE.MeshLambertMaterial({color:this.materialColor})
  this.sphereMaterial = new THREE.MeshNormalMaterial();
  
  THREE.ColorUtils.adjustHSV( this.planeMaterial.color, 0, 0, 0.9 );
...
CANNON.Demo.prototype.start = function(){
  ...
  function init() {
    ...
    that._scene = scene;
    scene.fog = new THREE.Fog( 0x220022, 1000, FAR );
    ...
    light = new THREE.PointLight( 0xf0faff,0.6 );
    light.position.set( 10, 30, 30 );
    var light2 = new THREE.PointLight( 0xfff0fa,0.6 );
    light2.position.set( 30, 10, 10 );
    // light.target.position.set( 0, 0.5, 1 );
    if(that.shadowsOn){
  ...
  if(window.dat!=undefined){
    ...
    rf.add(that.settings,'rendermode',{Wireframe:that.renderModes.WIREFRAME,Solid:that.renderModes.NORMAL}).onChange(function(mode){
...
CANNON.Demo.prototype._buildScene = function(n){
  // this.renderMode = this.renderModes.WIREFRAME;
  var that = this,
  ...
  function shape2mesh(shape){
    ...
    case CANNON.Shape.types.SPHERE:
      var sphere_geometry = new THREE.SphereGeometry( shape.radius, 10, 10);
      mesh = new THREE.Mesh( sphere_geometry, that.sphereMaterial );
      break;
    ...
    case CANNON.Shape.types.PLANE:
      ...
      var ground = new THREE.Mesh( geometry, that.planeMaterial );
